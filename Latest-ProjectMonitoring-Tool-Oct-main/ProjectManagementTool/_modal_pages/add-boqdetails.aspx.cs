...
namespace ProjectManagementTool._modal_pages
{
    public partial class add_boqdetails : System.Web.UI.Page
    {
            ...
            try
            {
                ...
                if(dsBOQDetails.Tables[0].Rows.Count>0)
                {
                   ...
                   // txtitem.Visible = true;
                    ...
                    DDLInspectionType.SelectedValue= dsBOQDetails.Tables[0].Rows[0]["Inspection_Type"].ToString();
                }
            }
            catch( Exception ex)
            {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                string selectedValue = DDLInspectionType.SelectedValue;
                if (selectedValue == "00")
                {
                    selectedValue = null;
                }
                int cnt = dbObj.UpdateBOQDetails(txtitemnumber.Text, txtdesc.Text, txtquantity.Value, txtUnit.Value, txtInrRate.Value, txtLocalInrAmount.Value, new Guid(hidUid.Value), txtDuties.Value, txtExWorks.Value, txtLocalTransport.Value, txtGST.Value, selectedValue);
                if (cnt > 0)
                {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int cnt =   dbObj.InsertBOQDetails(txtitemnumber.Text, txtdesc.Text, txtquantity.Value, txtUnit.Value, txtInrRate.Value, txtForgignJpyRate.Value,
                    ...
                    new Guid(projectUid), string.Empty, new Guid(workPackageUID), txtDuties.Value, txtExWorks.Value, txtLocalTransport.Value, txtGST.Value, selectedValue);
                if (cnt > 0)
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "CLOSE", "<script language='javascript'>parent.location.href=parent.location.href;</script>");
                }
