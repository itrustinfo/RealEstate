    ...
    public partial class _default : System.Web.UI.Page
    {
        ...
        private void LoadItemsGraph2()
        {
            ...
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                strScript.AppendLine("function drawChart() {");
                strScript.AppendLine("var data = google.visualization.arrayToDataTable([");
                strScript.AppendLine("['Months', 'Stock Used (Rs Crore)', 'Stock Remaining (Rs Crore)'],");

                // Iterate through the rows of the DataSet and add data to the JavaScript code
                string[] months = { "Oct 23", "Nov 23", "Dec 23", "Jan 24", "Feb 24", "Mar 24" };
                for (int i = 0; i < 12; i++)
                {
                    string resource = months[i];
                    decimal? planned = 0;
                    decimal? deployed = 0;
                    planned = 20 * (6 - (i - 1));
                    deployed = 15 * (6 - (i - 1));
                ...
                strScript.Append("colors: ['#b66dff', '#57c7d4'],");
                strScript.Append("fontSize: 11,");
                strScript.Append("bars: 'Vertical',");
                strScript.Append("annotations: {");
                strScript.Append("alwaysOutside: true,");
                strScript.Append("textStyle: {");
                strScript.Append("fontSize: 12,");
                ...
                strScript.Append("axes: {");
                strScript.Append("x: {");
                strScript.Append("0: { side: 'bottom', label: 'Last 6 Months Stocks' }"); // Top x-axis.
                strScript.Append("}");
                strScript.Append("},");
                strScript.Append("hAxis: {");
                strScript.Append("minValue: 0");
                ...
                strScript.AppendLine("var chart = new google.charts.Bar(document.getElementById('chart_divItemStocks'));");
                strScript.AppendLine("chart.draw(data, google.charts.Bar.convertOptions(options));");
                strScript.AppendLine("};");
                strScript.AppendLine("</script>");
        ...
        private void LoadItemsGraph()
        {
            ...
            if (DDLWorkPackage.SelectedValue != "--Select--")
            {
                ...
                // strScript.AppendLine("<script type=\"text/javascript\" src=\"https://www.gstatic.com/charts/loader.js\"></script>");
                strScript.AppendLine("<script type=\"text/javascript\">");
                strScript.AppendLine("google.load('visualization', '1', {packages: ['corechart'],callback: drawchart1})");

                strScript.AppendLine("function drawchart1(dataValues)");
                strScript.AppendLine("{var data = new google.visualization.DataTable();");
                strScript.AppendLine("data.addColumn('string', 'KPI_MONTH');");
                strScript.AppendLine("data.addColumn('number', 'Inventory (Rs Cr.)','number','Qauntity')");
                strScript.AppendLine("data.addColumn('number', 'Steel (Rs Cr.)','number','Quantity')");
                strScript.AppendLine("data.addColumn('number', 'Cement (Rs Cr.)','number','Quantity')");
                strScript.AppendLine("data.addRow(['Apr-23', 100, 40, 60])");

                strScript.AppendLine("data.addRow(['May-23', 90, 30, 60])");

                strScript.AppendLine("data.addRow(['Jun-23', 80, 30, 50])");

                strScript.AppendLine("data.addRow(['Jul-23', 70, 20, 50])");

                strScript.AppendLine("data.addRow(['Aug-23', 60, 20, 40])");

                strScript.AppendLine("data.addRow(['Sep-23', 55, 15, 40])");

                strScript.AppendLine("data.addRow(['Oct-23', 50, 15, 35])");

                strScript.AppendLine("data.addRow(['Nov-23', 45,15, 30])");

                strScript.AppendLine("data.addRow(['Dec-23', 40, 10, 30])");

                strScript.AppendLine("data.addRow(['Jan-24', 35, 10, 25])");

                strScript.AppendLine("data.addRow(['Feb-24', 30, 8, 22])");

                strScript.AppendLine("data.addRow(['Mar-24', 25, 5, 20])");


                strScript.AppendLine("var chart = new google.visualization.ComboChart(document.getElementById('chart_divItemStocks'));");

                strScript.AppendLine("chart.draw(data,");
                strScript.Append("{title: 'Stock Items', position: 'top', fontsize: '14px',seriesType: 'bars',");
                strScript.Append("series:");
                strScript.Append("{");
                strScript.Append("1: { type: 'line' },");
                strScript.Append("2: { type: 'line' }");
                strScript.Append("},colors: ['#b66dff', '#57c7d4','#d63384'],");
                strScript.Append("chartArea: { width: '70%' },");
                strScript.Append("});}");
                strScript.AppendLine("</script>");
                //
                ltScript_ItemStocks.Text = strScript.ToString();
